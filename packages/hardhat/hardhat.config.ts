...
import { FactoryOptions } from "@nomiclabs/hardhat-ethers/dist/src/helpers";
...
task("wallet", "Create a wallet (pk) link", async (_, { ethers }) => {
  ...
  const privateKey: string = randomWallet._signingKey().privateKey;
...
task("fundedwallet", "Create a wallet (pk) link and fund it with deployer?")
  ...
  .setAction(async (taskArgs: any, { network, ethers }) => {
    const randomWallet: Wallet = ethers.Wallet.createRandom();
    const privateKey: string = randomWallet._signingKey().privateKey;
    console.log("🔐 WALLET Generated as " + randomWallet.address + "");
    let url = taskArgs.url ? taskArgs.url : "http://localhost:3000";

    let localDeployerMnemonic: Buffer | string = "";
    try {
    ...
    if (localDeployerMnemonic) {
      let deployerWallet: Wallet = ethers.Wallet.fromMnemonic(
...
async function addr(
  ethers: {
    ...
    providers?: typeof import("@ethersproject/providers");
    Contract?: typeof Contract;
    ContractFactory?: typeof ContractFactory;
    BigNumber?: typeof BigNumber;
    FixedNumber?: typeof FixedNumber;
    constants?: typeof import("@ethersproject/constants");
    errors?: typeof errors;
    logger?: utils.Logger;
    utils?: typeof utils;
    wordlists?: { [locale: string]: Wordlist };
    version?: "ethers/5.0.17";
...
function send(
  ...
  return signer.sendTransaction(txparams).then().catch();
...
task("send", "Send ETH")
  ...
  .setAction(async (taskArgs: any, { network, ethers }) => {
    ...
    debug(parseInt(txRequest.gasPrice) / 1000000000 + " gwei");
